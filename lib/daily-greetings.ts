...
export async function triggerDailyGreetings() {
  try {
    ...
    if (birthdayMembers.length > 0) {
      ...
      const bcastHtml = `<div style="text-align:center;padding:16px"><div style="font-size:40px;margin-bottom:12px">🎉🎂🌸</div><h2 style="color:#2d6a4f;font-size:20px;margin:0 0 12px">Birthday Celebration${plural} Today!</h2><p style="font-size:14px;color:#555">Let us wish our fellow member${plural} a very happy birthday:</p><ul style="list-style:none;padding:0;text-align:left;display:inline-block">${namesHtml}</ul></div>`;
