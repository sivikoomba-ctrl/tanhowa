...
import { Download, FileText, IndianRupee, Users, CheckCircle, Clock, AlertTriangle, FileDown, BarChart3, ListTodo, Activity, TrendingUp, Lightbulb, MessageSquareWarning, Award, Receipt, XCircle } from "lucide-react";
...
function ExpensesTab() {
  ...
  function downloadExpensePDF() {
    ...
    if (byCategory.length > 1) {
      autoTable(doc, {
        ...
        body: byCategory.map((c) => [
          c.category, c.count, c.approved, c.pending, c.rejected,
          c.approvedAmount.toLocaleString("en-IN"), c.pendingAmount.toLocaleString("en-IN"),
        ]),
        theme: "grid",
    ...
    if (byOfficial.length > 1) {
      autoTable(doc, {
        ...
        body: byOfficial.map((o) => [
          o.name, o.official_type === "state" ? "State" : "District", o.count, o.approved, o.pending, o.rejected,
          o.approvedAmount.toLocaleString("en-IN"),
        ]),
        theme: "grid",
        headStyles: { fillColor: [45, 106, 79], fontSize: 8 },
        bodyStyles: { fontSize: 7 },
    ...
    autoTable(doc, {
      ...
      body: vouchers.map((v, i) => [
        i + 1, v.title, v.submitter_name, v.category || "—", v.vendor_name || "—", v.invoice_number || "—",
        (v.amount || 0).toLocaleString("en-IN"), v.status,
        v.expense_date ? new Date(v.expense_date).toLocaleDateString("en-IN") : "—",
      ]),
      theme: "grid",
      headStyles: { fillColor: [45, 106, 79], fontSize: 8 },
      bodyStyles: { fontSize: 7 },
  ...
  function downloadExpenseCSV() {
    ...
    const rows = vouchers.map((v) => [
      v.title, v.submitter_name, v.submitter_type, v.category, v.vendor_name, v.invoice_number,
      v.amount, v.status, v.expense_date || "", v.created_at,
    ]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-end gap-2">
        <Button variant="outline" onClick={downloadExpensePDF} disabled={vouchers.length === 0}>
          <FileDown size={14} className="mr-2" /> PDF
        </Button>
        <Button variant="outline" onClick={downloadExpenseCSV} disabled={vouchers.length === 0}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <Card>
          <CardContent className="pt-4 pb-4 flex items-center gap-3">
            ...
            <div><p className="text-2xl font-bold">{summary.total}</p><p className="text-xs text-muted-foreground">Total Vouchers</p></div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 pb-4 flex items-center gap-3">
            <CheckCircle className="w-8 h-8 text-green-500/60" />
            <div><p className="text-2xl font-bold text-green-700">₹{summary.totalAmount.toLocaleString("en-IN")}</p><p className="text-xs text-muted-foreground">Approved ({summary.approved})</p></div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 pb-4 flex items-center gap-3">
            <Clock className="w-8 h-8 text-amber-500/60" />
            <div><p className="text-2xl font-bold text-amber-700">₹{summary.pendingAmount.toLocaleString("en-IN")}</p><p className="text-xs text-muted-foreground">Pending ({summary.pending})</p></div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 pb-4 flex items-center gap-3">
            <XCircle className="w-8 h-8 text-red-500/60" />
            <div><p className="text-2xl font-bold text-red-700">{summary.rejected}</p><p className="text-xs text-muted-foreground">Rejected</p></div>
          </CardContent>
        </Card>
      </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-center">Total</TableHead>
                    <TableHead className="text-center text-green-700">Approved</TableHead>
                    <TableHead className="text-center text-amber-700">Pending</TableHead>
                    <TableHead className="text-center text-red-700">Rejected</TableHead>
                    ...
                    <TableRow key={v.id}>
                      <TableCell>
                        <div>
                          ...
                          {v.invoice_number && <p className="text-xs text-muted-foreground">Invoice: {v.invoice_number}</p>}
                        </div>
                      </TableCell>
