...
| `chat_channels` | Group chat channels | name, description, is_default, archived, pinned_message_id, created_by |
| `chat_channel_members` | Per-channel membership + state | channel_id, user_id, role (member/admin), muted, last_read_at, joined_at |
| `chat_messages` | Group chat messages | channel_id, sender_id, content, file_url, file_name, file_type, reply_to, deleted_at, edited_at |
...
- **APIs:**
  ...
  - `/api/chat/messages` — GET (cursor pagination via `?before=`, optional `?q=` for in-channel ILIKE search, signed file URLs 1hr TTL, reply snippets, conditional `last_read_at` update only when newer messages exist), POST (text via JSON, files via FormData 10MB max), PATCH `{ id, content }` (edit own text message — stamps `edited_at`, broadcasts `message_edited`; file messages not editable), DELETE (soft-delete + remove storage object so prior signed URLs stop working). Accepts both `?channel_id=` and `?channel=`.
  - `/api/chat/pin` — GET `?channel_id=` returns the channel's currently pinned message (hydrated with sender name). POST `{ message_id }` pins a message to its channel (replaces any existing pin). DELETE `?channel_id=` unpins. Restricted to site admins + the channel's own `role=admin` members — regular members can't self-promote their own messages. Broadcasts `message_pinned` with `{ messageId }` (or `{ messageId: null }` on unpin). Stale pins (message deleted) are auto-cleared when GET resolves them.
...
- **Realtime (Stage 2+):** Supabase Broadcast via `lib/chat-broadcast.ts` — server sends, anon client subscribes to `chat:${channelId}` topic. Events: `new_message`, `message_deleted`, `message_edited`, `message_pinned`, `reaction_added`, `reaction_removed`, `typing`, `read`. DB RLS not required — broadcast events live on the Realtime server, not in Postgres. Self-echo dedup via `knownMessageIdsRef` (messages) and `payload.userId === myIdRef.current` (others). `message_pinned` payload only carries `{ messageId }`; clients re-fetch `/api/chat/pin` to hydrate sender name and preview.
- **Stage 4 features (edit / pin / search):**
  - **Edit:** Sender-only. Hover Pencil on own text message → inline textarea replaces bubble content; Enter saves, Shift+Enter newline, Escape cancels. Server stamps `edited_at`; UI renders "(edited)" with hover tooltip of edit time. File messages are not editable.
  - **Pin:** Site admins + channel-admins only (gated via `chat_channel_members.role='admin'` or `isAdmin(session)`). Pin button in hover actions. Pinned message shows as amber banner above the message thread with sender name + snippet; unpin button visible to moderators. One pin per channel — new pin replaces the old.
  - **Search:** Search icon in thread header toggles a search bar; Enter runs `/api/chat/messages?channel=X&q=needle` (ILIKE with escaped `%`/`_`). Results replace the thread with plain rows (sender + timestamp + content). Close button (or Escape) clears and returns to the live thread. Scales via the `idx_chat_messages_content_trgm` GIN trigram index.
