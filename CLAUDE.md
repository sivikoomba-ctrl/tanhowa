...
- `posting_details` JSONB fields: `regular_district`, `regular_block`, `regular_farm` (optional — TN horticulture facility name, "Farms (if applicable)" dropdown), `special_duty_district`, `special_duty_block`, `special_duty_place`, `special_designation` ("HO Tech (State Scheme)" / "HO Tech (GOI)" / "Farm Manager"), `special_farm` (TN horticulture farm name, shown when Farm Manager selected), `deputed_district`, `deputed_block`. When adding new subfields, update the `PostingDetails` interface and `emptyPosting` constant in both `app/onboarding/page.tsx` and `app/dashboard/profile/page.tsx` — no DB migration needed (JSONB).
