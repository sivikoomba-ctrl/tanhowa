...
- **`localStorage`/`sessionStorage` can be `null`, not just throw.** In privacy-hardened browsers (e.g. Firefox with `dom.storage.enabled=false`), `window.localStorage` evaluates to `null` instead of throwing a `SecurityError`, so a bare `localStorage.getItem(...)` crashes with `Cannot read properties of null`. Confirmed in production via `error_logs` (Firefox 141, path `/`) — the crash was in `lib/i18n/language-context.tsx`, which wraps the whole app in `LanguageProvider`, so it took the entire site down for affected users, not just one feature. Fixed there with try/catch-wrapped `safeGetItem`/`safeSetItem` helpers (2026-08-02). Any **new** storage access outside a component-scoped `try {} catch {}` (most call sites in `components/*.tsx` already guard individually) should follow the same pattern, especially anything mounted near the app root.
...
- **`/admin/ai-space/satellite-ndvi`** — pulls a Sentinel-2 NDVI (or Sentinel-1 RVI, cloud-penetrating radar) time series for an uploaded/pasted field-boundary KML from the Copernicus Data Space Ecosystem, detects rise-peak-fall crop cycles, and charts it. TypeScript port of the `field-ndvi-analysis` skill's Python scripts (`tools/fetch_ndvi.py` / `tools/fetch_s1_rvi.py` at the project root, one level up from `tanhowa/`) — **`lib/sentinel-hub.ts`** (`parseKmlPolygon`, `fetchNdviTimeSeries`, `fetchRviTimeSeries`, `findCropCycles` — a JS approximation of `scipy.signal.find_peaks(prominence=...)`) + **`POST /api/ai-tools/satellite-ndvi`** (super_admin-gated, rate-limited 10/min, `SH_CLIENT_ID`/`SH_CLIENT_SECRET` required in `.env.local` + Vercel). Contribution action: `used_ai_satellite_ndvi`.
  ...
  - **Field Analysis Report card** (modes `ndvi`/`combined` only, not pure `rvi`) — surfaces sowing date, harvest date, crop duration, an estimated-harvest projection for an in-progress cycle, growth-stress windows, and field/crop area, with a "Download Field Analysis Report" PDF button (`jsPDF`+`jspdf-autotable`, same `headStyles: {fillColor:[45,106,79]}` pattern as `app/admin/roster/page.tsx`). Implementation: `computePolygonAreaHectares()` (equirectangular projection + shoelace, outer-boundary-only since `parseKmlPolygon()` doesn't extract inner rings/holes — flagged in the UI, not silently wrong; also runs an O(n²) self-intersection sanity check on the boundary) and `detectCropCycles(series, peaks)`, both in `lib/sentinel-hub.ts`. Phenology methodology (Start-of-Season/End-of-Season, a real remote-sensing technique):
    - **Cycle boundaries between two major peaks are the local valley between them** — not an independent threshold walk per peak. This is what makes double-cropping (two majors with no dip below any single fixed baseline) resolve correctly; a threshold-crossing approach alone silently merges them into one wrong cycle. Verified via `.tmp/` smoke test against synthetic double-cropping data before shipping.
    - **Unbounded ends** (before the first major peak / after the last) use a **relative** per-cycle threshold (`valley + 0.3*(peak-valley)`), not a hardcoded absolute NDVI value — field-to-field bare-soil/fallow baselines vary too much for one constant to work everywhere.
    - **Crossing dates are linearly interpolated** between the two bracketing samples (not snapped to the nearest one) — with monsoon gaps the brackets can be 2-4 weeks apart, so this materially improves precision. The bracket width becomes `dateUncertaintyDays`, shown in the UI above 7 days; this also covers the **interior valley case** via `sampleIsolation()` — a valley bracketed by a huge data gap (e.g. an off-season with no fetched samples at all) is a weak boundary estimate even though it's a "real" observed point, and the uncertainty number is what surfaces that.
    - **Perennial/orchard detection** (`isPerennialPattern`): if a cycle's bracketing valleys never drop meaningfully below the peak (stays above ~0.4, or amplitude <0.15), sowing/harvest/duration/stress-window are suppressed rather than reporting a plausible-looking wrong date for a continuous canopy crop (mango, coconut, etc.).
    - **Estimated Harvest** (ongoing cycles only, i.e. season hasn't returned to baseline by the end of the fetched range): borrows the duration from this field's *same-season* completed cycle (closest sowing month — TN fields commonly rotate crop types/durations across seasons, so an unrelated season's duration is worse than not estimating) rather than a hardcoded crop-type duration table (deliberately not built — would fabricate numbers for an unverified crop). If completed-cycle durations disagree by >35%, or none exist, reports `insufficient-history` instead of a confident-looking single date.
    - **⚠️ Known gotcha (real bug found + fixed via the smoke test, not hypothetical):** the harvest-side threshold for the *last* major peak must be computed from the cycle's own sowing-side baseline value, **not** a valley re-derived from the declining tail after the peak — a tail-derived valley is by construction always below its own self-referential threshold, so a crossing would *always* be "found" and `harvestConfidence: "ongoing"` could never actually trigger. If you touch this function, keep reusing the sowing-side baseline for the unbounded-right-end case.
    - **Growth-stress window** (reframed from "pest/disease" — NDVI can't diagnose a cause): within the rise phase only, a dip of ≥0.08 that persists ≥2 samples before recovering. UI/PDF disclaimer explicitly lists non-biotic causes (water stress, waterlogging, fertilization timing, cloud/atmospheric artifact) alongside pest/disease — "verify on the ground," never a diagnosis.
...
## WhatsApp Notifications (`lib/whatsapp.ts`)

Meta WhatsApp Cloud API scaffolding — deployed and safe to build against, but **live-but-silent in production** since the account-level prerequisites haven't been completed yet (this is a business/procurement gap, not a code bug).

- **`whatsappEnabled()`** gates every send on `WHATSAPP_PHONE_NUMBER_ID` + `WHATSAPP_ACCESS_TOKEN` being set — both are unset in Vercel, so `sendWhatsAppTemplate()` always returns `false` today. It's designed this way deliberately ("safe to deploy ahead of Meta onboarding" per the module's own doc comment) — new call sites can be wired now without waiting.
- **`toWhatsAppNumber()`** normalizes a raw phone field to Meta's `91XXXXXXXXXX` form; returns `null` for anything that isn't a plausible 10-digit Indian number (bare 10-digit, `0`-prefixed 11-digit, or already-prefixed 12/13-digit).
- **Opt-in:** `notification_prefs.whatsapp` (default `false`, compliance-safe), toggle on `/dashboard/profile` → Notification Preferences. Callers must check this themselves — `sendWhatsAppTemplate()` only handles delivery, not consent.
- **Templates are Meta-approved, not free text** — business-initiated WhatsApp messages must use a pre-submitted template name + ordered `{{1}}, {{2}}…` body params (`sendWhatsAppTemplate(phone, templateName, bodyParams, languageCode?)`). No template has been submitted to Meta yet, so even with credentials set, `payment_confirmed` (the one referenced in code) would still fail until approved.
- **Live call site:** subscription approval in `app/api/subscriptions/route.ts` (`PUT`, `status: "paid"`) fires `payment_confirmed` (`{{1}}` name, `{{2}}` period, `{{3}}` amount) gated on the member's opt-in.
- **Full plan** (provider comparison, cost estimate, prerequisite checklist, remaining templates) lives in the project-root `workflows/whatsapp-integration.md`. Activation is blocked entirely on business steps (register a WhatsApp Business Account or BSP, complete Meta Business verification, get templates approved) — none of that is code work.
- **Built 2026-06-15** (`0403bac`); the commit flagged **dues-reminder WhatsApp in `tools/subscription_reminder.py`** as the next slice once the first live site proved the pattern.
...
- **`add-contribution`** (POST action, member-allowed) creates a NEW pending flexible row → a *ledger* of contributions (pay multiple times). Member page shows a purple per-fund "Add Contribution" card with the total contributed. **DB constraint gotcha (fixed 2026-08-02):** `subscriptions` originally had a full `UNIQUE(user_id, period)` constraint, which silently 500'd every second contribution to the same fund (confirmed zero members had ever succeeded at a repeat contribution before the fix). It's now a **partial** unique index — `CREATE UNIQUE INDEX subscriptions_user_period_unique ON subscriptions (user_id, period) WHERE flexible_amount IS NOT TRUE` — so flexible rows are exempt from the dedup and regular (non-flexible) subscriptions still can't duplicate. Any code that inserts into `subscriptions` with `flexible_amount: true` can now safely create multiple rows for the same `(user_id, period)`. Non-flexible inserts are still fully protected — the partial index enforces `(user_id, period)` uniqueness for them exactly as the old full constraint did, races included — so `bulk-create`/`member-create`'s existing dupe pre-checks remain correct, just now paired with real DB-level enforcement instead of a constraint that also (wrongly) applied to flexible rows.
...
Admin-only aggregation of member-submitted social links at `/admin/social-media` + `/api/admin/social-media` (`isAdmin`-gated GET/PUT, mirrors `/api/roster`'s gate). All four `users.social_links` platform fields in one export/copy view, plus a pinned card for TANHOWA's own official account.

- **Member rows:** built from `users.social_links.{instagram,twitter,linkedin,whatsapp}` via `fetchAllRows()`, junk-value filtered (`nil`, `n/a`, the bare platform name, etc.) — no attempt to verify or auto-correct a handle against the real account (see the flagged full-name-instead-of-handle values, which are left as-is for the member to fix, not guessed at). Excludes test accounts.
- **UI:** metric cards (count per platform), search + per-platform filter, per-cell copy button + outbound link (best-effort URL built from a bare handle for Instagram/Twitter/WhatsApp; LinkedIn only links when the stored value is already a full URL), "Copy list" bulk-copies the current filtered view, Excel export via `lib/export-xlsx.ts`.
- **Official account card:** `OFFICIAL_ACCOUNTS` constant in the page (currently just `@tanhowa1979` on Instagram) — pinned separately from member rows since it's the association's own account, not a member's.
- **Manual metrics (no live API):** follower/post counts for the official account are hand-entered by an admin via a dialog, not pulled from Instagram's Graph API (that would need a Business-account-linked Facebook App with app review — out of scope). Stored as a single JSON blob in `site_settings` (key `official_social_metrics`: `{followers, posts, updated_at, updated_by}`), same pattern as `why_ministry_doc`. `PUT /api/admin/social-media` writes it, audit-logged `social_media_metrics_update`.
...
- **Logic lives in `lib/profile-completion.ts`** (`getMissingProfileFields()` / `isProfileComplete()`) — a shared source of truth used by three call sites: `app/dashboard/layout.tsx` (`getMissingFields` is now just an alias to the shared function, kept for its existing call-site names), the admin overview "Profile Updated" metric card (`/api/reports/overview`), and the admin Member Approval **"Profile Status" filter** (`/admin/users` — filter to 100%-complete or incomplete, plus an "Export PDF" button that includes a Missing Fields column when not filtered to complete-only). Update all three by editing the one shared function — don't re-duplicate the field list.
- **Placeholder name detection:** `PLACEHOLDER_NAMES` Set in `lib/profile-completion.ts` rejects names where every word is a placeholder (`unnamed`, `user`, `test`, `guest`, `anonymous`, `no name`, `n/a`, `na`). So "unnamed", "user user", "test test" all count as missing — the member is forced to enter a real first + last name. Add new placeholders to the Set rather than altering the field check.
